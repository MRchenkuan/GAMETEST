...
function Game(option) {
    ...
    game.init = function () {
        ...
        // 洗牌
        game.stack_action.sort(function () {
            return Math.random() > .5 ? -1 : 1;
        });
        game.stack_zouzhang.sort(function () {
            return Math.random() > .5 ? -1 : 1;
        });
        game.stack_guanzhi.sort(function () {
            return Math.random() > .5 ? -1 : 1;
        });
        game.stack_guansheng.sort(function () {
            return Math.random() > .5 ? -1 : 1;
        });
        game.stack_mengsheng.sort(function () {
            return Math.random() > .5 ? -1 : 1;
        });
    ...
    game.timer = {
        ...
        // 开始计时
        startCount: function () {
            if(this.startTime) throw "计时已开始,请勿重复开始";
            this.startTime = Date.now();
        },
        startTime: null,
        progress: 0
